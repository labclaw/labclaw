...
def show_landing() -> None:
    ...
    # Scientific method circular flow
    st.markdown(
    ...
    # System layers
    st.markdown('<div class="section-title">Architecture Layers</div>', unsafe_allow_html=True)
    _loop_desc = (
    ...
    st.markdown(f'<p class="version-tag">LabClaw v{__version__}</p>', unsafe_allow_html=True)
...
def show_discovery() -> None:
    ...
    if st.button("Run Discovery", type="primary", key="run_discovery_btn"):
        ...
        _run_discovery_pipeline(csv_path, dataset_choice if not custom_path else custom_path)
...
def _run_discovery_pipeline(csv_path: Path, dataset_name: str) -> None:
    ...
    st.markdown('<div class="section-title">Step 1: Data Loaded</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-title">Step 2: Patterns Mined</div>', unsafe_allow_html=True)
    ...
    # Step 4: Statistical validation (p-value display)
    st.markdown(
    ...
    if result.patterns:
        ...
        if sig_count:
            st.success(
                f"{sig_count} of {len(val_rows)} pattern(s) "
...
def show_scientific_loop() -> None:
    ...
    if results:
        ...
        # Findings provenance
        st.markdown('<div class="section-title">Last Cycle Details</div>', unsafe_allow_html=True)
        ...
        # All cycles table
        if len(results) > 1:
            st.markdown('<div class="section-title">All Cycles</div>', unsafe_allow_html=True)
            import pandas as pd
    ...
    # Run button
    st.markdown('<div class="section-title">Run a Cycle</div>', unsafe_allow_html=True)
...
def show_evolution() -> None:
    ...
    st.markdown('<div class="section-title">Fitness Timeline</div>', unsafe_allow_html=True)
    ...
    if selected_target:
        ...
        if scores:
                ...
                [
                    {
                        "Measurement": idx + 1,
                        "Avg Fitness": (
                            sum(s.metrics.values()) / len(s.metrics) if s.metrics else 0.0
                        ),
            ...
            if latest.metrics:
                st.caption(
                    "Latest: " + " | ".join(f"{k} = {v:.4f}" for k, v in latest.metrics.items())
                )
        else:
            st.caption("No fitness measurements for this target yet. Run a demo cycle below.")

    # Stage progression horizontal pipeline
    st.markdown(
        '<div class="section-title">Stage Progression Pipeline</div>', unsafe_allow_html=True
    )
    stage_colors = {
    ...
    for col, stage in zip(scols, stage_order):
        ...
        col.markdown(
            ...
            f"""<div class="metric-label">{stage.value}</div>"""
            """</div>""",
            unsafe_allow_html=True,
        )
    ...
    # History table
    if history:
        st.markdown('<div class="section-title">Cycle History</div>', unsafe_allow_html=True)
        import pandas as pd
    ...
    # Demo button
    st.markdown('<div class="section-title">Run Evolution Demo</div>', unsafe_allow_html=True)
    if st.button("Run Evolution Demo Cycle", type="primary", key="evo_demo_btn"):
        ...
        if candidates:
            ...
            st.success(
                ...
                f"(accuracy: 0.75 -> 0.78, recall: 0.80 -> 0.82)"
            )
            st.rerun()
...
def show_memory() -> None:
    ...
    # Memory statistics
    st.markdown('<div class="section-title">Memory Statistics</div>', unsafe_allow_html=True)
    ...
    api_sections = [
        ...
        (
            ...
            [
                ...
                ("POST", "/api/discovery/hypothesize", "Generate hypotheses from patterns"),
            ],
        ),
        (
            "Evolution",
            [
                ...
                ("GET", "/api/evolution/fitness/{target}", "Fitness history for a target"),
            ],
        ),
        (
            "Memory",
